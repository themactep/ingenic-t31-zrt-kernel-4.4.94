...
static int _ipu_set_osd_chx_route(struct jz_ipu *ipu, struct ipu_param *ip, int ch)
{
	...
	if (isrgb == 1) {
		ipu_reg_write(ipu, IPU_OSD_IN_CH0_Y_STRIDE+0x10*ch, srcw*4);
	} else if (isrgb == 2){        //The format is RGBA5551
		ipu_reg_write(ipu, IPU_OSD_IN_CH0_Y_STRIDE+0x10*ch, srcw*2);
	} else {
		ipu_reg_write(ipu, IPU_OSD_IN_CH0_Y_STRIDE+0x10*ch, srcw);
		ipu_reg_write(ipu, IPU_OSD_IN_CH0_UV_STRIDE+0x10*ch, srcw);
	}
...
static const struct of_device_id ingenic_ipu_dt_match[] = {
	{ .compatible = "ingenic,t31-ipu", .data = NULL },
