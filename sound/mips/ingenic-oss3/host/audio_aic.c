...
MODULE_PARM_DESC(mono_channel, "mic record channel select:1->left channel 2->right channel");
...
static int init_pipe(struct audio_pipe *pipe, enum dma_data_direction direction, unsigned long iobase, int dma_type)
{
	...
	if (DMA_FROM_DEVICE == direction) {
		...
		if (INGENIC_DMA_REQ_I2S0_TX == dma_type || INGENIC_DMA_REQ_I2S0_RX == dma_type)
			pipe->dma_config.src_addr = iobase + AICDR;
		else if (INGENIC_DMA_REQ_AEC == dma_type)
...
static int set_codec_mic_again(struct audio_aic_device *aic, void *data)
{
	...
	if (aic->codec_mic_info->alc_en_l == 1)
		again.lmode = CODEC_ANALOG_AUTO;
	else
		again.lmode = CODEC_ANALOG_MANUAL;

	if (aic->codec_mic_info->alc_en_r == 1)
		again.rmode = CODEC_ANALOG_AUTO;
	else
		again.rmode = CODEC_ANALOG_MANUAL;
	...
	if (gain.channel == 1)
		aic->codec_mic_info->lagain = again.gain.gain[0];
	else if (gain.channel == 2)
		aic->codec_mic_info->ragain = again.gain.gain[1];
	else if (gain.channel == 3) {
		aic->codec_mic_info->lagain = again.gain.gain[0];
		aic->codec_mic_info->ragain = again.gain.gain[1];
	}
...
static int set_codec_mic_dgain(struct audio_aic_device *aic, void *data)
{
	...
	else if (dgain.channel == 2)
		aic->codec_mic_info->rdgain = dgain.gain[1];
	else if (dgain.channel == 3) {
		aic->codec_mic_info->ldgain = dgain.gain[0];
		aic->codec_mic_info->rdgain = dgain.gain[1];
	}
...
static int set_codec_mic_alc_gain(struct audio_aic_device *aic, void *data)
{
	...
	if (aic->livingcodec->record->set_alc_threshold)
		aic->livingcodec->record->set_alc_threshold(alc);

	if (alc.channel == MONO_LEFT) {
		aic->codec_mic_info->lalc_maxgain = alc.maxgain[0];
		aic->codec_mic_info->lalc_mingain = alc.mingain[0];
	} else if (alc.channel == MONO_RIGHT) {
		aic->codec_mic_info->ralc_maxgain = alc.maxgain[1];
		aic->codec_mic_info->ralc_mingain = alc.mingain[1];
	} else {
		aic->codec_mic_info->lalc_maxgain = alc.maxgain[0];
		aic->codec_mic_info->lalc_mingain = alc.mingain[0];
		aic->codec_mic_info->ralc_maxgain = alc.maxgain[1];
...
{
	int ret = 0;
	struct channel_mute mute = *(struct channel_mute*)data;
	...
	if (mute.channel != MONO_LEFT && mute.channel != MONO_RIGHT &&
			mute.channel != STEREO) {
		audio_err_print("mute channel param is invalid %ld.\n", mute.channel);
		return -1;
	}

	if (mute.channel == MONO_LEFT) {
...
static int mic_ioctl(void *pipe, unsigned int cmd, void *data)
{
	...
	switch(cmd) {
		...
		case AUDIO_CMD_GET_ALC_GAIN:
			{
				...
				if (vol->channel == MONO_LEFT) {
					vol->maxgain[0] = cur_codec->lalc_maxgain;
					vol->mingain[0] = cur_codec->lalc_mingain;
				} else if (vol->channel == MONO_RIGHT) {
					vol->maxgain[1] = cur_codec->ralc_maxgain;
					vol->mingain[1] = cur_codec->ralc_mingain;
				} else {
					vol->maxgain[0] = cur_codec->lalc_maxgain;
					vol->mingain[0] = cur_codec->lalc_mingain;
					vol->maxgain[1] = cur_codec->ralc_maxgain;
		...
		case AUDIO_CMD_ENABLE_ALC_L:
			cur_codec->alc_en_l = *(int*)data;
			break;

		case AUDIO_CMD_ENABLE_ALC_R:
			cur_codec->alc_en_r = *(int*)data;
			break;
...
static int set_codec_spk_datatype(struct audio_aic_device *aic, void *data)
{
	...
	//set channel
	if (MONO_LEFT == data_type.sample_channel)
		__i2s_enable_mono2stereo(aic);
	__i2s_out_channel_select(aic, data_type.sample_channel-1);
...
static int set_codec_spk_again(struct audio_aic_device *aic, void *data)
{
	...
	if (gain.channel == 1)
		aic->codec_spk_info->lagain = again.gain.gain[0];
	else if (gain.channel == 2)
		aic->codec_mic_info->ragain = again.gain.gain[1];
	else if (gain.channel == 3) {
		aic->codec_spk_info->lagain = again.gain.gain[0];
		aic->codec_spk_info->ragain = again.gain.gain[1];
	}
...
static int set_codec_spk_mute(struct audio_aic_device *aic, void *data)
{
	...
	aic->codec_spk_info->lchannel_mute_en = mute.mute_en;
	if (aic->livingcodec->playback->set_mute)
		ret = aic->livingcodec->playback->set_mute(mute.channel, mute.mute_en, aic->codec_spk_info->ldgain);
...
static int init_dma_config(struct audio_aic_device *aic)
{
	...
	if (init_pipe(mic_pipe, DMA_FROM_DEVICE, aic->res->start, INGENIC_DMA_REQ_I2S0_RX) != 0) {
	...
	if (init_pipe(spk_pipe, DMA_TO_DEVICE, aic->res->start, INGENIC_DMA_REQ_I2S0_TX) != 0) {
	...
	if (init_pipe(aec_pipe, DMA_FROM_DEVICE, aic->res->start, INGENIC_DMA_REQ_AEC) != 0) {
...
#define EXTERNAL_CODEC 1
	...
	__i2s_clear_ror(aic);
	__i2s_clear_tur(aic);
	__i2s_set_receive_trigger(aic, DEFAULT_RECORD_TRIGGER-1);
	__i2s_set_transmit_trigger(aic, DEFAULT_REPLAY_TRIGGER);
	__i2s_write_tfifo_loop(aic, DEFAULT_AEC_TRIGGER-1);
